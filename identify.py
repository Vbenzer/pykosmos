...
def identify_dtw(arc, ref, display=False, upsample=False, Ufactor=5,
    ...
    alignment = dtw.dtw((x1.flux.value - np.nanmedian(x1.flux.value)) / np.nanmax(x1.flux.value),
                        (x2.flux.value - np.nanmedian(x2.flux.value)) / np.nanmax(x1.flux.value),
                        keep_internals=True, step_pattern=step_pattern,
                        window_type=window_type,
                        open_begin=open_begin, open_end=open_end, window_args=window_args)
    if display:
    ...
    if display:
        ...
        # Change the x axis from the arc to very roughly match the reference
        print(f"max x2: {np.max(x2.spectral_axis.value)}, min x2: {np.min(x2.spectral_axis.value)}"
              f", max x1: {np.max(x1.spectral_axis.value)}, min x1: {np.min(x1.spectral_axis.value)}")

        x1_axis_align = (x1.spectral_axis.value / max(x1.spectral_axis.value)
                         * (max(x2.spectral_axis.value) - min(x2.spectral_axis.value))
                         + min(x2.spectral_axis.value))

        # (x1.flux.value - np.nanmedian(x1.flux.value)) / np.nanmax(x1.flux.value)
        #(x2.flux.value - np.nanmedian(x2.flux.value)) / np.nanmax(x2.flux.value)

        plt.plot(x1_axis_align, (x1.flux.value - np.nanmedian(x1.flux.value)) / np.nanmax(x1.flux.value),
                 label='Arc (normalized)')
        plt.plot(x2.spectral_axis.value, (x2.flux.value - np.nanmedian(x2.flux.value)) / np.nanmax(x2.flux.value)-0.01,
                 label='Reference (normalized)')
        for i, j in list(zip(alignment.index1, alignment.index2))[::10]:
            plt.plot([x1_axis_align[i], x2.spectral_axis.value[j]],
                     [((x1.flux.value - np.nanmedian(x1.flux.value)) / np.nanmax(x1.flux.value))[i],
                      ((x2.flux.value - np.nanmedian(x2.flux.value)) / np.nanmax(x1.flux.value))[j]-0.01],
    ...
    # Catch zeros in wav_guess
    zerook = np.where((wav_guess == 0))[0]

    # Give positions of zeros
    if len(zerook) > 0:
        # Interpolate over zeros
        good = np.where((wav_guess != 0))[0]
        interp_func = interp1d(x1.spectral_axis.value[good], wav_guess[good], kind='linear', fill_value='extrapolate')
        wav_guess[zerook] = interp_func(x1.spectral_axis.value[zerook])
        print("Interpolated over zero points in wav_guess.")
    ...
    pks, wpks = find_peaks(wav_guess, x1.flux.value,
                           use_scipy=True, pthreshold=pthreshold)

    # Plot the peaks found in the arc spectrum with the actual spectrum
    if display:
        plt.figure(figsize=(10, 5))
        plt.plot(arc.spectral_axis.value, arc.flux.value, label='Arc Spectrum')
        plt.scatter(pks, np.zeros(len(pks)), color='red', label='Peaks Found', s=10, alpha=0.7)
        plt.xlabel('Pixel / Wavelength')
        plt.ylabel('Flux')
        plt.title('Peaks Found in Arc Spectrum')
        plt.legend()
        plt.show(block=False)
...
def fit_wavesol_isotonic(pixels, wavelengths):
    from sklearn.isotonic import IsotonicRegression

    # 1) sort by pixel
    idx = np.argsort(pixels)
    x = np.asarray(pixels)[idx]
    y = np.asarray(wavelengths)[idx]

    # 2) isotonic (non-decreasing) piecewise-linear fit
    iso = IsotonicRegression(increasing=True, out_of_bounds="clip")
    y_iso = iso.fit_transform(x, y)

    # y_iso is your monotone solution at the sample pixels

    # To evaluate on arbitrary pixels xq:
    def wavesol(xq):
        # linear interpolation on (x, y_iso), monotone by construction
        return np.interp(xq, x, y_iso)

    return wavesol, (x, y_iso)



def fit_wavesol_isotonic_pchip(pixels, wavelengths):
    from scipy.interpolate import PchipInterpolator

    # step 1: isotonic
    wavesol_iso, (x, y_iso) = fit_wavesol_isotonic(pixels, wavelengths)
    # step 2: smooth + still monotone
    pchip = PchipInterpolator(x, y_iso, extrapolate=False)
    ...
    def wavesol(xq):
        ...
        # left extrapolation
        mask_left = xq < pixels[0]
        if np.any(mask_left):
            slope_left = max((wavelengths[1] - wavelengths[0]) /
                             (pixels[1] - pixels[0]), 1e-12)
            yq[mask_left] = wavelengths[0] + slope_left * (xq[mask_left] - pixels[0])

        # right extrapolation
        mask_right = xq > pixels[-1]
        if np.any(mask_right):
            slope_right = max((wavelengths[-1] - wavelengths[-2]) /
                              (pixels[-1] - pixels[-2]), 1e-12)
            yq[mask_right] = wavelengths[-1] + slope_right * (xq[mask_right] - pixels[-1])

        return yq

    return wavesol, pchip


def robust_monotone_wavesol(pixels, wavelengths):
    from sklearn.linear_model import HuberRegressor

    x = np.asarray(pixels); y = np.asarray(wavelengths)
    idx = np.argsort(x); x = x[idx]; y = y[idx]

    # Robust linear trend just to flag gross outliers (works even if global trend isn't linear)
    X = x[:, None]
    huber = HuberRegressor().fit(X, y)
    resid = y - huber.predict(X)
    keep = np.abs(resid) < 6*np.std(resid)

    wavesol, model = fit_wavesol_isotonic_pchip(x[keep], y[keep])
...
def fit_wavelength(spec, xpoints, wpoints, display=False,
    ...
    if mode.lower() == 'poly':
        # Use HuberRegressor to remove gross outliers before polyfit
        from sklearn.linear_model import HuberRegressor
        X = xpt[:, None]
        huber = HuberRegressor().fit(X, wpt)
        resid = wpt - huber.predict(X)
        keep = np.abs(resid) < 1 * np.std(resid)
        fit = np.polyfit(xpt[keep], wpt[keep], deg)
        wavesolved = np.polyval(fit, spec.spectral_axis.value)
        fpt = np.polyval(fit, xpoints)

    if mode.lower() == 'iso':
        # Fit a piecewise linear isotonic regression
        wavesolved, (x_iso, y_iso) = fit_wavesol_isotonic(xpt, wpt)
        fpt = wavesolved(xpoints)
        wavesolved = wavesolved(spec.spectral_axis.value)

    if mode.lower() == 'iso_pchip':
        # Fit a piecewise linear isotonic regression with PCHIP smoothing
        wavesolved, pchip = fit_wavesol_isotonic_pchip(xpt, wpt)
        fpt = wavesolved(xpoints)
        wavesolved = wavesolved(spec.spectral_axis.value)

    if mode.lower() == 'robust_iso':
        # Fit a robust monotone isotonic regression
        wavesolved = robust_monotone_wavesol(xpt, wpt)
        fpt = wavesolved(xpoints)
        wavesolved = wavesolved(spec.spectral_axis.value)
    ...
    if display:
        plt.figure()
        plt.plot(spec.spectral_axis.value, wavesolved, label='Wavelength Fit')
        plt.scatter(xpt, wpt, c='r', label='Input Points')
    ...
    outspec = Spectrum(spectral_axis=wavesolved * wpoints.unit,
